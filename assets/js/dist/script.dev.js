...
function switchAnimals() {
  ...
  if (begin < end) setTimeout("switchAnimals()", 10000);else if (begin == end) {
    begin = 0;
    setTimeout("switchAnimals()", 10000);
  }
} //---------------------------------------------------
...
var data = fetch('http://localhost:3000/animal/list').then(function (response) {
...
var textSearch = document.getElementById("tip-search"); // textSearch.addEventListener('keyup', function (){
...
function clickC() {
  ...
  fetch("http://localhost:3000/animal/".concat(selectAnimal.key, "/").concat(selectAnimal.gender, "/").concat(selectAnimal.phylum, "/").concat(selectAnimal.grade, "/").concat(selectAnimal.family, "/").concat(selectAnimal.set)).then(function (response) {
    return response.json();
  }).then(function (data) {
    ...
    data.map(function (element) {
      var imageVideo = [];
      element.imageOrVideo.map(function (result) {
        if (result != null) {
      ...
      wrap.appendChild(div0); // set apia
      //hinh anh

      img.src = imageVideo[0]; //ten tieng anh

      linknode.innerText = element.scienceName; //linknode.target = "_blank"
      //linknode.href =            
      ...
      div4.innerHTML = element.vietnameseName || ''; // Moi Truong song 

      div5.innerHTML = element.habitat || ''; //Tim thay 
